...
/** @ingroup algorithms **/
template<typename Range, typename Body>
  void parallel_for( const Range& range, const Body& body, affinity_partitioner& partitioner, const char* file_name, int line_number  ) {
  internal::start_for<Range,Body,affinity_partitioner>::run(range,body,partitioner, file_name, line_number, __builtin_return_address(0));
}
