...
namespace internal {
    ...
    class final_sum: public t_debug_task {
    public:
        Body my_body;
    private:
    ...
    /** @ingroup algorithms */
    template<typename Range, typename Body>
    class sum_node: public t_debug_task {
    ...
    private:
	  ...
	  __exec_begin__(getTaskId());
            ...
            if( my_body ) {
                ...
                my_body = NULL; 
                if( a ) spawn(*b, __FILE__, __LINE__);
                else a = b;
    ...
    /** @ingroup algorithms */
    template<typename Range, typename Body>
    class finish_scan: public t_debug_task {
    ...
    class start_scan: public t_debug_task {
        typedef sum_node<Range,Body> sum_node_type;
        typedef final_sum<Range,Body> final_sum_type;
        final_sum_type* my_body;
        ...
        static void run( const Range& range_, Body& body_, const Partitioner& partitioner_ ) {
            if( !range_.empty() ) {
                ...
                start_pass1_type& pass1 = *new(task::allocate_root()) start_pass1_type(
                    ...
                    *temp_body,
                    partitioner_ );
                t_debug_task::spawn_root_and_wait( pass1, __FILE__, __LINE__ );
                if( root ) {
                    root->my_body = temp_body;
                    root->my_incoming = NULL;
                    root->my_stuff_last = &body_;
                    t_debug_task::spawn_root_and_wait( *root, __FILE__, __LINE__ );
                } else {
