...
namespace tbb {
  ...
  public:
    static void spawn( task& t, const char* file, int line );__attribute__((optimize(0)));
  ...
  inline void t_debug_task::spawn(task& t, const char* file, int line) {
  ...
  inline void t_debug_task::spawn_root_and_wait( task& root, const char* file, int line, void* ret_address, bool par_for ) {
    ...
    if (ret_address == 0) {
      taskGraph->CaptureSetTaskId(get_cur_tid(), (static_cast<t_debug_task&>(root)).getTaskId(), false, __builtin_return_address(0), file, line);
    } else {
      taskGraph->CaptureSetTaskId(get_cur_tid(), (static_cast<t_debug_task&>(root)).getTaskId(), false, ret_address, file, line);
    }
  ...
  inline void t_debug_task::spawn_and_wait_for_all( task& child,const char* file, int line ) {
    ...
    taskGraph->CaptureSetTaskId(get_cur_tid(), (static_cast<t_debug_task&>(child)).getTaskId(), true, __builtin_return_address(0), file, line);
